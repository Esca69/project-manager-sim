...
func _close():
	visible = false
	# --- Снимаем паузу (если мы её поставили) ---
...
func _build_ui():
	...
	# === Главное окно (как EmployeeRoster: PanelContainer) ===
	...
	# === ШАПКА (синяя, как EmployeeRoster) ===
	...
	# === КНОПКА (белый фон, синяя обводка, как в проекте) ===
...
func _build_finance_section():
	...
	_add_finance_row(grid, "Балан�� на конец дня:", "$%d" % balance_end, total_color, true)
...
func _build_projects_section():
	...
	if finished_today.size() > 0:
		...
		for entry in finished_today:
			...
			var text = "[%s] %s — Выплата: $%d" % [proj.category.to_upper(), proj.title, payout]
	...
	if failed_today.size() > 0:
		...
		for proj in failed_today:
			var text = "[%s] %s — Хард-дедлайн истёк, выплата: $0" % [proj.category.to_upper(), proj.title]
			var lbl = Label.new()
			lbl.text = text
	...
	if in_progress.size() > 0:
		...
		for proj in in_progress:
			...
			var text = "[%s] %s — %s | софт: %d дн. | хард: %d дн." % [
