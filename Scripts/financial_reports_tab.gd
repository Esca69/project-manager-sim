...
# Income/Expense bar graph node
var _bar_chart: Control
# Daily bars node
...
func _get_filtered_records() -> Array:
	...
	if _selected_period == 0 or all.size() <= _selected_period:
		return all.duplicate()
	return all.slice(all.size() - _selected_period)
...
func _draw_cash_flow(ctrl: Control):
	...
	var zero_y = pad_top + gh * (1.0 - (0 - min_val) / range_val)
	var n_lines = 5
	for i in range(n_lines + 1):
		...
		var val_at_line = int(max_val - frac * range_val)
		var label_str = "$%s" % _format_money_short(val_at_line)
	...
	for i in range(n):
		...
		var py = pad_top + gh * (1.0 - (values[i] - min_val) / range_val)
		pts.append(Vector2(px, py))

	# Blue fill area (above zero or above min)
	var fill_poly: PackedVector2Array = []
	var baseline_y = clampf(pad_top + gh * (1.0 - (0 - min_val) / range_val), pad_top, pad_top + gh)
...
func _build_structure_card() -> PanelContainer:
	...
	title_row.add_child(_make_title("💰 " + tr("REPORTS_INCOME_STRUCTURE") + " / " + tr("REPORTS_EXPENSE_STRUCTURE")))

	_bar_chart = Control.new()
	_bar_chart.custom_minimum_size = Vector2(0, 160)
	_bar_chart.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	_bar_chart.draw.connect(_draw_structure_bars.bind(_bar_chart))
	vbox.add_child(_bar_chart)

	return card

func _refresh_structure():
	if _bar_chart:
		_bar_chart.queue_redraw()

func _draw_structure_bars(ctrl: Control):
	var records = _get_filtered_records()
	var w = ctrl.size.x
	if records.is_empty():
		ctrl.draw_string(ThemeDB.fallback_font, Vector2(20, 60), tr("REPORTS_NO_DATA"), HORIZONTAL_ALIGNMENT_LEFT, -1, 14, COLOR_GRAY)
		return
	...
	var bar_h = 28.0
	var bar_y1 = 10.0
	var bar_y2 = bar_y1 + bar_h + 40.0
	var label_area = pad
	var bar_w = w - label_area - 20.0

	# Income bar (green)
	var income_segs = [
		{"label": tr("REPORTS_PROJECT_INCOME"), "value": total_income, "color": COLOR_GREEN},
	]
	_draw_stacked_bar(ctrl, income_segs, total_income, label_area, bar_y1, bar_w, bar_h)

	# Income label on left
	ctrl.draw_string(ThemeDB.fallback_font, Vector2(0, bar_y1 + bar_h * 0.5 + 5), tr("REPORTS_INCOME_STRUCTURE"), HORIZONTAL_ALIGNMENT_LEFT, label_area - 4, 11, COLOR_GREEN)

	# Expense bar (red spectrum)
	var total_exp = total_salary + total_pm_salary + total_penalties + total_office
	var exp_segs = [
		{"label": tr("REPORTS_SALARIES"),   "value": total_salary,    "color": COLOR_RED},
		{"label": tr("REPORTS_PM_SALARY"),  "value": total_pm_salary, "color": COLOR_ORANGE},
		{"label": tr("REPORTS_PENALTIES"),  "value": total_penalties, "color": Color(0.7, 0.1, 0.1, 1)},
		{"label": tr("REPORTS_OFFICE"),     "value": total_office,    "color": Color(0.9, 0.5, 0.1, 1)},
	]
	_draw_stacked_bar(ctrl, exp_segs, max(total_exp, 1), label_area, bar_y2, bar_w, bar_h)

	# Expense label on left
	ctrl.draw_string(ThemeDB.fallback_font, Vector2(0, bar_y2 + bar_h * 0.5 + 5), tr("REPORTS_EXPENSE_STRUCTURE"), HORIZONTAL_ALIGNMENT_LEFT, label_area - 4, 11, COLOR_RED)

	# Legend
	var legend_y = bar_y2 + bar_h + 8.0
	var lx = label_area
	for seg in exp_segs:
		if seg["value"] <= 0:
			continue
		var sq_size = 10.0
		ctrl.draw_rect(Rect2(lx, legend_y, sq_size, sq_size), seg["color"])
		ctrl.draw_string(ThemeDB.fallback_font, Vector2(lx + sq_size + 4, legend_y + 10), seg["label"], HORIZONTAL_ALIGNMENT_LEFT, -1, 11, COLOR_DARK)
		lx += 100.0
...
func _draw_daily_bars(ctrl: Control):
	...
	# Legend
	var legend_x = pad_left + gw - 200.0
	ctrl.draw_rect(Rect2(legend_x, pad_top, 12, 12), COLOR_GREEN)
	ctrl.draw_string(ThemeDB.fallback_font, Vector2(legend_x + 16, pad_top + 11), tr("REPORTS_PROJECT_INCOME"), HORIZONTAL_ALIGNMENT_LEFT, -1, 11, COLOR_DARK)
	ctrl.draw_rect(Rect2(legend_x + 120, pad_top, 12, 12), COLOR_RED)
	ctrl.draw_string(ThemeDB.fallback_font, Vector2(legend_x + 136, pad_top + 11), tr("REPORTS_EXPENSE_STRUCTURE"), HORIZONTAL_ALIGNMENT_LEFT, -1, 11, COLOR_DARK)
...
func _refresh_roi():
	...
	var row_h = 36.0
	var min_h = 60.0
	_roi_chart.custom_minimum_size = Vector2(0, max(min_h, projects.size() * row_h + 20.0))
