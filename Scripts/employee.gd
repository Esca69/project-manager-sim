...
enum State {
	...
	GOING_LUNCH_TABLE,
	LUNCH_EATING,
	# === DEPRECATED: старые стейты чата (не используются, сохранены для совместимости сейвов) ===
	GOING_TO_CHAT,  # DEPRECATED
	CHATTING,        # DEPRECATED
	# === VACATION SYSTEM ===
	ON_VACATION,
...
var _pending_training: bool = false
var _pending_unpaid_leave: bool = false
...
const CHAT_ALLOWED_STATES: Array = [
	...
	State.GOING_COFFEE,
	State.COFFEE_BREAK,
	State.GOING_LUNCH_FRIDGE,
	State.LUNCH_AT_FRIDGE,
	State.GOING_LUNCH_KITCHEN,
	State.LUNCH_AT_KITCHEN,
	State.GOING_LUNCH_TABLE,
	State.LUNCH_EATING,
	State.GOING_TOILET,
	State.MOVING,
	State.IDLE,
	...
	match current_state:
		...
		State.GOING_HOME:
			...
			if dist < 50.0:
				if is_on_day_off:
					_finalize_day_off()
				elif _pending_training:
					_finalize_training_departure()
				elif _pending_unpaid_leave:
...
func _execute_proximity_chat(partner_npc):
	...
	if result.is_positive:
		...
		EventLog.add(tr("LOG_CHAT_NEGATIVE") % [data.get_display_name(), partner_npc.data.get_display_name(), rel_val, level_name] + " | Δ%+d (%s)" % [result.delta, debug_parts])
...
func _set_pair_cooldown(partner_name: String, minutes: float):
...
func move_to_desk(target_point: Vector2):
	...
	if not is_internal_call:
		var do_not_disturb_states = [
			...
			State.HOME, State.GOING_HOME,
			State.GOING_COFFEE, State.COFFEE_BREAK,
			State.GOING_TOILET, State.TOILET_BREAK,
			State.GOING_LUNCH_FRIDGE, State.LUNCH_AT_FRIDGE,
			State.GOING_LUNCH_KITCHEN, State.LUNCH_AT_KITCHEN,
			State.GOING_LUNCH_TABLE, State.LUNCH_EATING,
